...
class drm_ajaxClass {
    ...
    function __construct() {
        ...
        add_action('wp_ajax_nopriv_show_search_result', array(&$this, 'show_search_result')); //no logged
        add_action('wp_ajax_show_search_result', array(&$this, 'show_search_result')); //logged 
    }
...
//                var_dump($loop_product->ID);
            ...
            ?>
<a href="<?php echo post_permalink($loop_product->ID) ?>">
            <span class="results-items col-sm-4 nopadding ">
                ...
                <div class="results-items col-sm-12 nopadding ">
                    <div class="row upcoming_tb_row">
                        ...
                        <div class="pull-right col-sm-6 col-md-8 items-right upcoming_tb_right">
                            ...
                            <div class="dealer">
                                Dealer: <?php echo $event_dealer; ?>
                            </div>
                            <div class="website"><a hre="<?php echo $event_website_name; ?>" target="_blank" rel="nofollow"><?php echo $event_website_name; ?></a></div>
...
//        }
    ...
    function show_dealers() {
        ...
        global $wpdb;
         if (($searchTextField != '') && ($prod_filt != '')) {
         $res_a = array();
           $query = $wpdb->get_results("SELECT * FROM `wp_postmeta` WHERE (meta_key='dealer_state' AND meta_value LIKE'%" . $searchTextField . "%') or (meta_key='dealer_city' AND  meta_value LIKE'%" . $searchTextField . "%') OR (meta_key='zip_code' AND meta_value ='" . $searchTextField . "')", 'ARRAY_A');
//            $query = $wpdb->get_results("SELECT * FROM `wp_postmeta` WHERE meta_key='dealer_state' AND meta_value='" . $searchTextField . "' or meta_key='dealer_city' AND  meta_value='" . $searchTextField . "'", 'ARRAY_A');
            ...
            foreach ($query as $key => $value) {
                ...
                $post_ids = get_posts($args);
                foreach($post_ids as $post_ids_1){
                    if(!isset( $res_a[$post_ids_1->ID] )) 
                    $res_a[$post_ids_1->ID] = 1;
                }
            }
            ...
            if (isset($p_id)) {
                foreach ($p_id as $p_id4) {
                    ...
                    <li class="dl_ulli" data-lat="<?php echo $lat; ?>" data-lang="<?php echo $lng; ?>">
                        <span class="uppercase ttl1"><?php echo $c_nm; ?></span><p><?php echo $address['address']; ?></p><p>T: <?php echo $telephone_d; ?></p><p>E:<a href="mailto:<?php echo $email_d; ?>" target="_blank" ><?php echo $email_d; ?></a> </p> <p>W:<a href="http://<?php echo $webiste_d; ?>" target="_blank" > <?php echo $webiste_d; ?></a></p><p>Opening Hours: <?php echo $opening_hours; ?></p>
                    </li>
      <?php   }
        else if ($searchTextField != '' && $prod_filt == '') {
             $query = $wpdb->get_results("SELECT * FROM `wp_postmeta` WHERE (meta_key='dealer_state' AND meta_value LIKE'%" . $searchTextField . "%') or (meta_key='dealer_city' AND  meta_value LIKE'%" . $searchTextField . "%') OR (meta_key='zip_code' AND meta_value ='" . $searchTextField . "')", 'ARRAY_A');
//            $query = $wpdb->get_results("SELECT * FROM `wp_postmeta` WHERE meta_key='dealer_state' AND meta_value='" . $searchTextField . "' or meta_key='dealer_city' AND  meta_value='" . $searchTextField . "'", 'ARRAY_A');
            $query_ = array();
            foreach ($query as $key => $value) {
            ...
            $query = $wpdb->get_results("SELECT * FROM wp_postmeta INNER JOIN wp_posts ON wp_postmeta.post_id = wp_posts.ID WHERE wp_posts.post_title = '" . $prod_filt . "' AND  wp_posts.post_status='publish' AND wp_posts.post_type='products'", 'ARRAY_A');
            $p_id2 = $query[0]['post_id'];        
//             var_dump("SELECT * FROM wp_postmeta INNER JOIN wp_posts ON wp_postmeta.post_id = wp_posts.ID WHERE wp_posts.post_title = '" . $prod_filt . "' AND  wp_posts.post_status='publish'");
            $query_ = array();
            $p_id = get_field('dealers_list', $p_id2);       
//               var_dump($p_id);
            if (isset($p_id)) {
...
//                            $i =0;
    ...
    $mypostids = $wpdb->get_results("SELECT ID FROM wp_posts WHERE post_type = 'products' AND post_title LIKE '%$search_text%' AND post_status='publish'");
    foreach($mypostids as $sec_ids){
        $s_id = $sec_ids->ID;
         $s_id1 = array($s_id);
         $s_id1 = implode(',', $s_id1);
            ...
            $args = array(
                ...
                foreach ($post_ids as $_post1) {
                    if (has_post_thumbnail($_post1->ID)) {
                        $url = wp_get_attachment_image_src(get_post_thumbnail_id($_post1->ID), 'thumbnail');
                    }
                    $v_id = $_post1->ID;
                     $sz_ft = get_field('size_feet', $v_id)? get_field('size_feet', $v_id) : "";
                        $sz_inch = get_field('size_inches', $v_id)? get_field('size_inches', $v_id) : "";
                    ?>
                  <li><a href="<?php echo get_permalink($_post1->ID); ?>"><span class="col-md-12"><span class="col-md-2"><?php echo "<img class='img-responsive' src='" . get_template_directory_uri() . "/timthumb.php?src=" . $url[0] . "&a=t&w=200&h=170&zc=1' alt=''>"; ?></span>
                  <span class="col-md-10"><h4><?php echo $_post1->post_title; ?></h4>
                  <span class="sc_met">
                        <p>$<?php echo get_field('price_thousands', $_post1->ID); ?>.<?php echo get_field('price_hundreds', $_post1->ID); ?></p>
                      <p>Size: <?php if($sz_ft != '' || $sz_inch !='') { ?><?php echo $sz_ft; ?>'<?php echo $sz_inch; ?>"<?php } ?></p><?php // echo get_field('search_fileds', $_post1->ID); ?>
                      <p>Occupants: <?php echo get_field('price_thousands', $_post1->ID); ?></p>
                      <p>Tare: <?php echo get_field('tare', $_post1->ID); ?></p>
                      <p>Ball Weight: <?php echo get_field('ball_weight', $_post1->ID); ?></p>
                  </span>
                  </span></span>
                 </a>
                  </li>
...
//            'to' => WPDREAMSEEKER_ADMIN_EMAIL,
    ...
    function send_make_a_disclaimer() {
        ...
        $claim_product = str_replace('_', ' ', $claim_product);
        $claim_model = isset($_POST['claim_model']) ? $_POST['claim_model'] : "";
        $claim_model = str_replace('_', ' ', $claim_model);
        ...
        $claim_dealer = str_replace('_', ' ', $claim_dealer);
